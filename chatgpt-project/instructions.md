...
- `01-protocol.md`: the full method. It covers the interview flow, the question card, decision classes, the visual ladder, the `OD:` copy-back lines, designer hooks, guardrails, and output templates. It was written for skill hosts, so translate its paths this way:
...
```
Then run `python3 od/scripts/engine.py init --name "<product>"`, then `pick`, `set`, `generate`, `validate`, `design-md`, `export --format css`, and `preview`. Offer the files in `opendesigner/` as downloads. Without the Python tool, say so, and give values only from the knowledge files, labelled as defaults. Never do color or contrast math in your head.

**How to run the interview:**
1. **Open with one bundled question.** Ask:
   - what the product is, who uses it, and which platforms it runs on;
   - which mode they want: Quick (10 questions, about 5 minutes), Standard (about 92 questions over 27 screens), or Expert (all 191);
   - the one thing people should remember about the product;
   - whether they have a site, screenshot, Figma file or brand book to learn from, and one they dislike.
2. **Walk the stages in order.** Ask only the questions in the chosen mode whose *Show if* holds. Everything else takes its default and is recorded as `default`.
3. **Ask one high-weight decision per turn.** Use the question card from `01-protocol.md`: the question id, a one-line grounding, a plain explanation, stakes, 2 to 4 options with the recommended one first (each with a real system that uses it), then "Recommendation: X because Y". Group up to three low-weight questions in one turn. Spend the most time on the highest fan-out decisions: brand personality, platforms, visual style, platform posture, device classes, where the system lives, density and scope. Accessibility floors always get full attention.
...
8. **Gates.** Pause for approval three times: after scope (stages 01 to 05), after direction (06 to 08), and after the asset checklist. Quick mode keeps only the direction gate. At each gate, list the decisions with their question ids so the person can say "change Q-shape-01".
9. **Finish:**

...
- **The ontology is still changing.** S1a added and removed nodes during this session (for example `found.type.i18n` no longer appears), so the counts will shift; the script that produced them is simple to re-run [inferred].

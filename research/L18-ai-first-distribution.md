...
Y = verified, N = verified absent or not supported, ? = unverified. "Claude app" = claude.ai web, Claude Desktop chat and mobile (Cowork and chat began merging into it on 16 Sep 2026, rolling out to Pro and Max first, then Team and Free [S-L18-038] [S-V1a-002]).
...
| **Interactive UI in chat** | Y: MCP Apps on web, desktop, mobile [S-L18-013]; custom visuals (web + desktop; click sends a follow-up) [S-L18-041]; artifacts with storage and connectors [S-L18-027]; Claude Design canvas [S-L18-029] | CLI: N for MCP Apps (falls back to text) [S-L18-044]; AskUserQuestion options [S-L18-304]; publishes claude.ai artifacts [S-L18-043]. Desktop Code tab: Y, renders MCP App widgets (already in the standard app when the changelog added them to third-party-platform builds on 2026-08-25; the standard-app date is not documented) [S-L18-045] [S-V1a-022] | N (the integrator renders) [inferred] | Y: MCP Apps widgets ("fully compatible" 2026-02-22) with follow-up messages and tool calls; code-block previews of HTML/React/SVG with no channel back [S-L18-055, S-L18-121, S-L18-128] | Y: MCP App panels; built-in browser renders localhost and local HTML, users comment on elements [S-L18-109, S-L18-115] | N (image input only) [S-L18-137, S-L18-138] | Y: MCP Apps since 2.6 (2026-03-03) [S-L18-222, S-L18-062] | Y: MCP Apps since 1.109 (2026-02-04) [S-L18-220, S-L18-061] | N [S-L18-217] |
...
- **Format.** Required `name` (1-64 chars, lowercase letters, digits, hyphens, must match the folder; Claude also forbids "anthropic" and "claude") and `description` (1-1024 chars, what it does and when to use it; the claude.ai Help Center caps it at 200 chars for skills uploaded to the Claude app [S-V1a-009]). Optional `license`, `compatibility` (<=500 chars), `metadata`, `allowed-tools` (experimental) [S-L18-050, S-L18-001].
...
- **Claude Code** reads AGENTS.md natively from v2.1.277, but only if no CLAUDE.md exists; it falls back to CLAUDE.md-only on older versions and in the first session after an upgrade, and on Bedrock or with telemetry off only before v2.1.281 [S-V1a-075]. Anthropic's recommended bridge is a CLAUDE.md containing `@AGENTS.md`, and it says to prefer that import over a symlink on Windows [S-L18-036, S-L18-211].
...
1. **A closed, structured token source referenced by name.** DTCG 2025.10 is a stable W3C Community Group format with `$description` for intent and `$deprecated` instead of deletion [S-L18-324, S-L18-325]. In Sanity's eval of its own design-system docs (Aug 2026), the new JSON-structured docs alone took Sonnet 4.6 from 20% to 90% task success; of the 3 MCP tools, retrievable code examples helped most (93%, and 100% with lints), lint less [S-L18-332] [S-V1b-050]. Anthropic reports models overwrite JSON less often than Markdown [S-L18-327]. Single-vendor eval, so treat the numbers as indicative.
...
- **Adds:** L00 predates the Agent Plugins 1.0 standard (6 Aug 2026), the custom GPT retirement notice, and the Claude/Cowork merge (started 16 Sep 2026, still rolling out [S-V1a-002]) [S-L18-058, S-L18-118, S-L18-038].
- **No dispute found** on MCP Apps, Agent Skills or AGENTS.md in L00.

## Cross-lane notes
- **L11:** DC-L11-23 lists "Claude Design (17 Apr 2026)"; it now works inside Claude conversations and Claude Code (Sep 2026; beta on paid plans, off by default on Enterprise [S-V1a-003]), and design systems are artifacts [S-L18-029, S-L18-032]. OpenAI's plugins and Agent Plugins 1.0 are new distribution channels for DC-L11-23.
- **L16:** Claude's desktop Code tab now renders MCP App widgets (standard-app date not documented; the changelog added them to third-party-platform builds on 2026-08-25) [S-L18-045] [S-V1a-022]; Codex desktop has a built-in browser with element comments [S-L18-115]; both are preview surfaces for DC-L16-06.
